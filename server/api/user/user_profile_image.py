...
class UserProfileImage(Resource):
    ...
    def put(self):
        ...
        for file in args['profile_image']:
            # file : 파일이름 / 실제 이미지등 본문 분리.
            
            # 파일 이름 저장됨 => S3 버킷에 저장될 경로 생성에 활용. -> 중복 발생 소지 있음.
            # 파일 이름은 재가공 (누가_언제), 확장자(.jpg)만 가져다 사용.
            # ex. PC카카오톡 파일 전송 -> 다운로드 : 보낸파일이름을 무시하고, Kakao_???????.jpg 등으로 받아짐.
            ...
            new_file_name = f"{new_file_name}{file_extension}" # MySNS_123456asdf_12345.png 등 파일명 완성.
